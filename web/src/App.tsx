...
function App() {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (msg) => {
      ...
      if (data.type === "history") {
        ...
        setPlacements(resolveDebateLayout(created?.payload));
        setAgentStates(Object.fromEntries(resolveDebateLayout(created?.payload).map((p) => [p.agentId, { state: "idle" as const }])));
        ...
        refreshWorldStatus(selectedWorldId);
      }
    };
  ...
  function refreshWorldStatus(worldId: string) {
